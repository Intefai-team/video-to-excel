...
import gc
import logging
from moviepy.video.io.VideoFileClip import VideoFileClip  # Correct import
    ...
    try:
        ...
        try:
            ...
            clip.audio.write_audiofile(
                ...
                codec="pcm_s16le", 
                fps=16000,
                logger=None  # Disables moviepy logging
            )
...
def extract_info(text):
    ...
    # Enhanced name patterns with better handling of name variations
    name_patterns = [
        r"(?:hi|hello|hey)[, ]*(?:this is me|i am|my name is|myself) ([A-Z][a-z]+)",  # "Hi, this is me Payal"
        r"\bthis is me[, ]*([A-Z][a-z]+)\b",  # "this is me Payal"
        r"\bmy name is ([A-Z][a-z]+)\b",  # "my name is Payal"
        r"\bi am ([A-Z][a-z]+)\b",  # "I am Payal"
        r"\bmyself ([A-Z][a-z]+)\b",  # "myself Payal"
        r"\bcall me ([A-Z][a-z]+)\b",  # "call me Payal"
        r"\bi'm ([A-Z][a-z]+)\b"  # "I'm Payal"
    ]
    ...
    location_patterns = [
        ...
        r"\bdid \w+ in ([A-Z][a-z]+)\b",  # "did engineering in India"
        r"\bmoved to ([A-Z][a-z]+)\b",  # "moved to India"
        r"\bengineering in ([A-Z][a-z]+)\b"  # "engineering in India"
    ]
    
    # Name extraction - try all patterns
    for pattern in name_patterns:
        match = re.search(pattern, text, re.IGNORECASE)
        if match:
            data["name"] = match.group(1).strip()
            # Special case handling for name variations
            if data["name"].lower() == "pyle":
                data["name"] = "Payal"  # Correct common mispronunciation
            break
...
if __name__ == "__main__":
    logger.info(f"Starting server - Whisper: {whisper_model is not None}, FFmpeg: {check_ffmpeg()}")
